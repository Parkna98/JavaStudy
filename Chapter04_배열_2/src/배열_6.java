...
public class 배열_6 {
	public static void process() {
		...
		for(int i=0;i<com.length;i++) {
			...
			for(int j=0;j<i;j++) { //중복없는 난수
				if(com[i]==com[j]) {
					i--; // for문 처음으로 돌아가서 다시 com[i]값 할당
					break;
				}
			}
		}
		int[] user=new int[3]; // 사용자 입력값 => 3자리입력 => 잘라서 배열에 저장
		// 출력
//		System.out.printf("%d%d%d\n",com[0],com[1],com[2]); // => 붙여서 출력
		// 사용자 입력
		Scanner scan=new Scanner(System.in);
		while(true) {
			System.out.print("3자리정수를 입력:");
			int input=scan.nextInt();
			
			if(input<100 || input>999) {
				System.out.println("잘못된 입력입니다!!");
				continue; // 다시 while의 조건식으로 이동 
			}
