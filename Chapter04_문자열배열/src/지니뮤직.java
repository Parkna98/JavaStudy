...
public class 지니뮤직 {
	...
	public static void main(String[] args) throws Exception{
		...
		while(true) {
			...
			String msg=scan.next(); // 문자열 받을때 next() or nextLine()
			
			Document doc=Jsoup.connect(url).get(); // 해당url의 페이지의 소스코드를 모두 가져오는 기능
			Elements title=doc.select("table.list-wrap a.title");  // 제목이 태그로 되어있는데 태그를 가져오는 기능이 Elements
			for(int i=0;i<title.size();i++) {
				String name=title.get(i).text();
				if(name.contains(msg)) {
//				if(name.startsWith(msg)) {
//				if(name.endsWith(msg)) {
//				if(name.equalsIgnoreCase(msg)) //대소문자구분없이
				{ 
