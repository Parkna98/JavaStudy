		...
		라이브러리
			...
			파일, 네트워크 		   ㅣ 						ㅣ
			 				InputStream				OutputStream
			 				   ㅣ						ㅣ
			 		일반파일(.zip, .png ...)		 	  Writer => String자체를 저장 (한글저장시 주로 사용)
			 			=> 1byte => 바이트스트림
			 		문자저장(.txt, .dat ...)
			 			=> 2byte => 문자스트림
			 			** 영어외의 다른 언어
				...
				*** 스트림은 데이터 이동 통로
				...
				바이트 스트림 (1byte) => 업로드, 다운로드, 카페...
				  ...
				  **BufferedInputStream	  **BufferedOutputStream
				  => 메모리에 전체를 저장 (속도가 빠르다)
				  **ObjectInputStream	  **ObjectOutputStream => ObjectStream은 단위 1byte만
				  => 객체단위 저장 (***)
				문자 스트림은 (2byte) => 파일 제어(한글) => 파일 읽기/쓰기
					Reader				  Writer
					  ㅣ						ㅣ
				**FileReader			**FileWriter
				 	  ㅣ						ㅣ
			...
			***** 
				...
				문자스트림  (2byte씩 송수신) => 한글파일 읽기/저장
						 ** 한글은 한글자당 2byte => UTF-8 (Unicode Transfer Format) (Unicode:2byte, ASCII:1byte)
						    -------------------------- Git 언어 통일
				보조스트림  => 읽는 속도, 쓰는 속도를 빨리하기 위해 미리 메모리에 올려 놓고 시작
							String / StringBuffer(메모리를 임시로 만듬) 
						=> Buffered~
					...
					*** File => 일반 클래스
						...
						=> 주요 메소드
							...
							2. 파일 정보
								...
								= ***파일 크기 : long length() (바이트단위라 숫자가 클수도 있어서 return long형임)
								= 수정일 : long lastModified() (시간까지 들어오기때문에 return long형임)
							3. 파일 종류
								= 파일 : boolean isFile()
								= 폴더 : boolean isDirectory()
							4. 파일, 디렉토리 만들기
								= createNewFle() => 파일 만들경우
								= ***mkdir() => 폴더까지 만듬
							5. 삭제
...
public class MainClass_File {
	...
	public static void main(String[] args) {
		...
		try {
			// directory 확인
			File dir=new File("c:\\javaDev"); // 해당경로에 있는 모든 파일읽기
			// window => \   , mac, linux => /
			// \는 반드시 \\를 사용한다
			// 가급적이면 절대경로를 이용한다
