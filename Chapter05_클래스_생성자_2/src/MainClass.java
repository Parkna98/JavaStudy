 ...
 * 		프로그램 기획
 * 			벤치마킹 / 요구사항 분석
 * 			------------------ 변수/메소드 => java는 메소드먼저 / C언어는 변수먼저
 * 		예)
 * 			숫자 야구게임
 * 			멤버변수 
 * 				=> com[], user[], s (*중복으로 쓰이는 변수들을 멤버변수로 설정)
 * 			컴퓨터 난수 발생 => 3개 => com[]
 * 			사용자가 입력 => 3개 => user[]
 * 			비교 => com[] user[]
 * 			힌트 => user[], s, b
 * 			종료 => s
 * 			====> 기능을 먼저 잡고 필요한 변수를 생각 => 중복으로 쓰이는 변수들을 멤버변수로 선언
 * 												=> 중복이 없으면 지역변수로
 * 				
 */
...
class User{
	...
	int[] arr=new int[5]; // 멤버변수 => 난수발생? => 구현? => 생성자!
